...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get chat history for the current user and device
  app.get("/api/chat/history", async (req: Request, res: Response) => {
    try {
      ...
      // Get chat history for this specific user and device
      let chatHistory = await storage.getChatHistory(userId, deviceId);
      
      // If no chat history exists for this device, create a new one
      if (!chatHistory) {
        chatHistory = await storage.createChatHistory({
          userId,
          messages: [{
            role: 'assistant',
            content: 'Hello! I\'m your EcoSense AI assistant. How can I help you today?'
          }]
        }, deviceId);
      }
  ...
  app.post("/api/chat/message", async (req: Request, res: Response) => {
    try {
      ...
      // Get existing chat history for this specific user and device
      let chatHistory = await storage.getChatHistory(userId, deviceId);
      ...
      const aiResponse = await getGroqCompletion(message, chatHistory.messages);
      ...
      const updatedMessages = [...chatHistory.messages, userMessage, assistantMessage];
      await storage.updateChatHistory(userId, { messages: updatedMessages }, deviceId);
